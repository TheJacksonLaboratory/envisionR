...
#' annotation <- read_annotation_csv(csv = annotation_csv, tzone = "US/Pacific")
#'
...
read_annotation_csv <- function(csv,
                                metadata = NULL,
                                tzone = NULL) {
  ...
  annotation_tibble <- read_annotation_csv_raw(csv, tzone)
  return(annotation_tibble)
}

#' Low-level internal read_annotation_csv
#' @param csv File path for the Envision® CSV.
#' @param tzone Time zone for the study. If left as \code{NULL}, the function will attempt to assume a time zone and throw a warning.
#' @noRd
read_annotation_csv_raw <- function(csv, tzone = NULL) {
  # Ensuring required packages are loaded
  stopifnot(requireNamespace("readr", quietly = TRUE))
  stopifnot(requireNamespace("janitor", quietly = TRUE))
  stopifnot(requireNamespace("dplyr", quietly = TRUE))
