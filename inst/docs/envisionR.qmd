...
* [Understanding Circadianism and Extracting Trend and Cycle](#understanding-circadianism-and-extracting-trend-and-cycle): How to extract important components of cyclical datasets.
...
We will use `tidyverse`, `janitor`, and `here`, all available from CRAN and installable using `install.packages()`, for data import. Note that we will make frequent use of tidy data and the pipe operator `|>` in R. Pipes and tidy data manipulation facilitate rapid transformation and analysis of data frame type objects in R.

Additionally, all assignment operators for global variables used here are `<-` and all functions from packages outside of base R utilize the `{package}::{function}()` format to ensure that you know which R package contains each function used in this vignette. This includes functions from `envisionR`.
...
It is often useful to create a URL to visualize the specific moments of video highlighted in an analysis. Such videos may allow the end user to look at exemplary or aberrant moments in a study. These may be used to illustrate activities of interest. We can also use the process of making a URL to look at time and how it is encoded in R. We will use time frequently here.
...
The study's app URL contains some important information. Each study has its own `org` ID and `study` ID. These IDs can help us interact with the Envision app.

We use a function called `envision_metadata()`. This does some checks of our input, some data type conversion under the hood, and acts as a convenient source for study information in downstream steps.
...
**NOTE: at the cage level, none of these variables has been normalized for cage occupancy.** Unnormalized data may have issues when occupancy changes. In each dataset, it is necessary to evaluate whether cage occupancy might play an effect.
...
We can change the units relatively simply with the function `velocity_unit_convert()`. For example, if we wish to know meters per hour (m/h), we can make a new calculated column with this information.
...
Normalizing for cage occupancy may make the distributions match one another better than they currently do. Normalization of the data can be performed by dividing `movement_mean_per_cage_cm_s_hour` by `animals_cage_quantity`.
...
Exploratory analysis of JAX Envision data frequently starts out with visualization of the dataset with respect to experimental factors of interest. This approach often yields good insights about the data that can be supported statistically.
...
This allows us to see specific moments of interest in long studies.
...
### A Note on Exploratory vs. Hypothesis-Driven Data Analysis
...
We see that for this specific comparison, 16 mg/kg caffeine produces a statistically significant change in activity. Recall our warning about HARKing: this hypothesis was generated from looking at the data, so we should take care when we interpret this finding.

How should we interpret a finding from HARKing? One important check is the prior evidence of biological plausibility of a mechanism and the degree to which the finding agrees or conflicts with that prior evidence. In this example, our finding states that caffeine increases activity in mice for a few hours during the time where they are usually asleep. This is an uncontroversial statement since we already know that caffeine is a stimulant. The only new thing we are saying is that Envision can detect this in the four hours following caffeine dosing. We might be fully willing to accept this result without further scrutiny.

Imagine, however, that we were testing a new compound and that Envision data were the first *in vivo* data for that compound. In this case, there is no prior expectation of a specific finding. The interpretation of the finding should be more circumspect. It may be better to interpret these data as a pilot showing when to look for an effect in a replication experiment.

In either case, you should rarely make this decision alone. It is best to seek out adversarial opinions on the plausibility and next steps from the experiment. If your colleagues believe you are over-interpreting the data, you should take their critiques seriously.

We can use the same code as above, but change the dose from `dose1` to `dose2`.
...
The ideal method of overcoming the effect of daylight time is to design experiments without daylight time present in the dataset. That means one of two things:
...
This vignette uses the statistics and data analysis programming language R. If you are a complete novice R user, you may consider attending a [Carpentries workshop](https://carpentries.org/) on R.
...
* [`ggplot2`](https://ggplot2.tidyverse.org/), a grammar for data visualization.
