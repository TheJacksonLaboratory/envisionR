...
test_that("get_utc_offset() returns character for whole number standard time", {
  expect_equal(get_utc_offset(
    as.POSIXct("2024-01-01 12:00:00",
      tz = "US/Pacific"
    ),
    as_numeric = FALSE
  ), "-08:00")
...
test_that("get_utc_offset() returns character for whole number daylight time", {
  expect_equal(get_utc_offset(
    as.POSIXct("2024-07-01 12:00:00",
      tz = "US/Pacific"
    ),
    as_numeric = FALSE
  ), "-07:00")
...
test_that("get_utc_offset() works for a UTC time stamp passed to the function.", {
  expect_equal(get_utc_offset(
    ts = as.POSIXct("2024-01-01 12:00:00",
      tz = "US/Pacific"
    ),
...
utc_datetimes <- as.POSIXct(paste0(
  "2024-01-20 ",
