...
pnw = structure(c(`Postnatal Week 0` = 19742, `Postnatal Week 1` = 19749,
...
test_that("weeks_postnatal_seq() returns expected results with as.Date() input for dob", {
  expect_equal(weeks_postnatal_seq(dob = as.Date("2024-01-20"),
                      n_weeks = 16),
               pnw)
})

test_that("weeks_postnatal_seq() returns expected results with character input for dob when in quiet", {
  expect_equal(weeks_postnatal_seq(dob = "2024-01-20",
                                   n_weeks = 16,
                                   quiet = TRUE),
               pnw)
})
...
test_that("weeks_postnatal_seq() throws a warning for character vector", {
  expect_warning(weeks_postnatal_seq(dob = "2024-01-20",
                                     n_weeks = 16),
