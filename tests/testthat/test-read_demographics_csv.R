...
csv_demo_out <- read.csv(
  text = demographics_csv_example,
  na.strings = c("", "NA"),
  ...
  janitor::clean_names() |>
  dplyr::mutate(
    birth_date = as.POSIXct(birth_date, tz = "UTC"),
...
test_that("read_demographics_csv() returns expected tibble with default parameters", {
  expect_equal(
    read_demographics_csv(tempcsv_1),
    csv_demo_out |>
      mutate(
...
test_that("read_animalscages_csv() wrapper works", {
  expect_equal(
    read_animalscages_csv(tempcsv_1),
    csv_demo_out |>
      mutate(
        birth_date = as.Date(birth_date),
        death_date = as.Date(death_date)
...
test_that("read_animalsandcages_csv() wrapper works", {
  expect_equal(
    read_animalsandcages_csv(tempcsv_1),
    csv_demo_out |>
      mutate(
        birth_date = as.Date(birth_date),
        death_date = as.Date(death_date)
...
test_that("read_demographics_csv() returns properly formatted data when amend_groupcage is set to FALSE", {
  expect_equal(
    read_demographics_csv(tempcsv_1, amend_groupcage = FALSE),
    csv_demo_out |>
      mutate(
        birth_date = as.Date(birth_date),
        death_date = as.Date(death_date)
...
test_that("read_demographics_csv() returns properly formatted data when date_only is set to FALSE", {
  expect_equal(
    read_demographics_csv(tempcsv_1, date_only = FALSE),
    csv_demo_out |>
      dplyr::rename(
        group_id = group,
        cage_id = cage
...
test_that("read_demographics_csv() can use a different version", {
  expect_equal(
    read_demographics_csv(tempcsv_2, date_only = FALSE),
    csv_demo_out |>
      dplyr::rename(
        group_id = group,
        cage_id = cage
...
test_that("read_demographics_csv() throws an error for an invalid version", {
  expect_error(
    read_demographics_csv(tempcsv_3, date_only = FALSE),
